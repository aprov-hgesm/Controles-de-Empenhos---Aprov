...
export interface WarehouseDeliveryProjection {
  ...
  integratedInvoiceCount: number;
  operationalOnlyInvoiceCount: number;
...
function invoiceQuantity(invoice: Invoice): number {
  return invoice.items.reduce((total, item) => total + positive(item.quantity), 0);
}
...
export function buildWarehouseDeliveryProjection(
  ...
  const empenhoInvoices = invoices
    ...
    .map((invoice) => ({
      ...
      warehouseIntegrated: Boolean(invoice.warehouseIntegration),
    }))
    .sort((left, right) =>
  ...
  const base = {
    ...
    invoiceReceivedQuantity: empenhoInvoices.reduce((total, invoice) => total + invoice.quantity, 0),
    integratedInvoiceCount: empenhoInvoices.filter((invoice) => invoice.warehouseIntegrated).length,
    operationalOnlyInvoiceCount: empenhoInvoices.filter((invoice) => !invoice.warehouseIntegrated).length,
    stock,
  };
...
export function buildWarehouseDeliveryProjections(
  ...
  return empenhos
    ...
    .sort((left, right) => {
      ...
      const dateOrder = (left.nextDeliveryDate || '9999-12-31').localeCompare(
        right.nextDeliveryDate || '9999-12-31'
      );
      return dateOrder !== 0 ? dateOrder : left.empenhoId.localeCompare(right.empenhoId, 'pt-BR');
    });
}
...
export function validateWarehouseLogisticsSettings(
  ...
  if (
    ...
    && (
      ...
      || input.lowStockThreshold > 1_000_000_000
    )
