...
function MetricCard({ label, value, detail, href, testId }: {
  label: string;
  value: string | number;
  detail: string;
  href: string;
...
export function WarehouseLogisticsDashboard({ workspaceId }: { workspaceId: string }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      setContext(next);
      // Alertas pertencem ao ADM. Falha de sincronização nunca impede o Dashboard.
    ...
    <div className="mt-6 space-y-5" data-testid="warehouse-logistics-dashboard">
      <div className="flex flex-col gap-4 rounded-2xl border border-blue-300/10 bg-blue-400/[0.035] p-5 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-400">
            Indicadores derivados das fontes reais do EMPROVEX e do ADM. Cronogramas,
            Empenhos e NFs são somente leitura; alertas e projeções permanecem no namespace warehouse.
          </p>
        </div>
      ...
      {context.truncated && (
        <div className="rounded-xl border border-amber-300/15 bg-amber-400/[0.05] px-4 py-3 text-xs leading-5 text-amber-100/80">
          Um dos limites de segurança foi atingido. O painel não conclui automaticamente
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <div className="rounded-2xl border border-white/[0.07] bg-black/10 p-5">
          ...
          <p className="mt-1 text-xs text-slate-600">Últimos registros dentro da consulta bounded.</p>
          <div className="mt-4 space-y-2">
            {recentInvoices.length ? recentInvoices.map((invoice) => (
              <div key={invoice.recordKey || invoice.id} className="flex items-start justify-between gap-3 rounded-xl border border-white/[0.05] px-3 py-3">
                ...
                <div className="text-right"><p className="text-[10px] text-slate-500">{dateTime(invoice.registeredAt || invoice.issueDate)}</p><p className="mt-1 text-[9px] text-slate-600">recebimento operacional EMPROVEX</p></div>
              </div>
        ...
        <div className="rounded-2xl border border-white/[0.07] bg-black/10 p-5">
          ...
          <p className="mt-1 text-xs text-slate-600">Ocorrências são derivadas de dados reais e persistidas somente no ADM.</p>
          <div className="mt-4 space-y-2">
            {context.alertCandidates.slice(0, 8).map((candidate) => (
              <div key={candidate.kind + ':' + candidate.entityId} className="rounded-xl border border-white/[0.05] px-3 py-3">
                <div className="flex items-start gap-2"><TriangleAlert className="mt-0.5 h-3.5 w-3.5 shrink-0 text-amber-200" /><div><p className="text-xs font-bold text-slate-300">{candidate.title}</p><p className="mt-1 text-[10px] leading-4 text-slate-600">{candidate.subtitle}</p></div></div>
              </div>
            ))}
