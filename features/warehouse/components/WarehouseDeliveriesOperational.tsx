    ...
    <div className="mt-6 space-y-5" data-testid="warehouse-deliveries-operational">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <div className="flex items-center gap-2 text-blue-200">
              ...
              <p className="text-xs font-black uppercase tracking-[0.12em]">Cronograma → NF → projeção logística</p>
            </div>
            <p className="mt-2 max-w-4xl text-sm leading-6 text-slate-400">
              O ADM lê Cronogramas, Empenhos e Notas Fiscais existentes sem alterá-los.
              O recebimento continua pertencendo ao EMPROVEX; a correlação com estoque usa
              exclusivamente os movimentos já registrados no namespace warehouse.
            </p>
          </div>
      ...
      {context?.truncated && (
        <div className="rounded-xl border border-amber-300/15 bg-amber-400/[0.05] px-4 py-3 text-xs leading-5 text-amber-100/80">
          A consulta atingiu um limite de segurança. Os dados exibidos continuam reais,
          mas a visão pode não abranger registros além da janela bounded.
        </div>
      )}

      {message && (
        <div className="rounded-xl border border-rose-300/15 bg-rose-400/[0.05] px-4 py-3 text-xs text-rose-100">
          {message}
        </div>
      )}
      ...
      {loading && !context ? (
        <div className="rounded-2xl border border-white/[0.07] bg-black/10 p-5 text-sm text-slate-400">
          Carregando Cronogramas, Empenhos, NFs e projeções do warehouse…
        </div>
      ) : !context?.projections.length ? (
        <div className="rounded-2xl border border-white/[0.07] bg-black/10 p-5 text-sm text-slate-400">
          Nenhum empenho foi encontrado na janela consultada.
        </div>
      ) : (
        <>
              ...
              <article key={delivery.empenhoId} data-testid="warehouse-delivery-card" data-empenho-id={delivery.empenhoId} className="rounded-2xl border border-white/[0.07] bg-black/10 p-5">
                ...
                <div className="mt-5 grid gap-4 lg:grid-cols-3">
                    ...
                    <div className="mt-3 space-y-2">
                      {delivery.invoices.length ? delivery.invoices.slice(0, 4).map((invoice) => (
                        <div key={invoice.recordKey} className="rounded-lg border border-white/[0.05] px-3 py-2">
                          ...
                          <p className="mt-1 text-[10px] text-slate-600">{dateLabel(invoice.registeredAt || invoice.issueDate)} · {invoice.projectedMaterialIds.length ? invoice.projectedMaterialIds.length + ' material(is) encontrado(s) no ledger ADM' : 'sem projeção logística identificada na janela'}</p>
                        </div>
                      )) : <p className="text-xs text-slate-600">Nenhuma NF encontrada para este empenho.</p>}
                    </div>
                  </div>
                  ...
                  <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] p-4">
                    ...
                    <div className="mt-3 space-y-2 text-xs text-slate-500">
                      <p>{delivery.stock.linkedMaterials} material(is) correlacionado(s) pelo ledger</p>
                      <p>{delivery.stock.positiveBalances} com saldo positivo · {delivery.stock.zeroBalances} zerado(s)</p>
                      <p>{delivery.stock.withoutBalance} sem saldo materializado na janela consultada</p>
                    </div>
                  </div>
                </div>
                ...
                {delivery.overdueQuantity > 0 && (
                  <div className="mt-4 flex items-start gap-2 rounded-xl border border-rose-300/12 bg-rose-400/[0.04] px-4 py-3 text-xs text-rose-100/80">
                    ...
                    O atraso é calculado cumulativamente por item. Nenhuma NF é associada artificialmente a uma remessa específica.
                  </div>
                )}
