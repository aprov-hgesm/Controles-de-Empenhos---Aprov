...
export function RelatorioPorFornecedorView({ empenhos, invoices }: RelatorioPorFornecedorViewProps) {
  ...
  const empenhosWithInvalidCnpj = React.useMemo(
    () => empenhos.filter((empenho) => Boolean(empenho.supplierCnpj?.trim()) && !isValidSupplierCnpj(empenho.supplierCnpj)).length,
    [empenhos]
  );
  ...
  const filteredSuppliers = React.useMemo(() => {
    ...
    const cnpjQuery = query.toUpperCase().replace(/[.\/\-\s]/g, '');
    ...
    return supplierReports.filter((report) => {
      ...
      return (
        ...
        (cnpjQuery.length > 0 && report.cnpj.includes(cnpjQuery)) ||
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-rose-100 bg-white p-4 shadow-sm">
          <CircleAlert className="mb-2 h-5 w-5 text-rose-600" aria-hidden="true" />
          <p className="text-2xl font-black text-[#0b1c30]">{empenhosWithInvalidCnpj}</p>
          <p className="text-xs font-bold text-gray-500">CNPJs históricos inválidos</p>
        </div>
      </div>

      {empenhosWithInvalidCnpj > 0 && (
        <div className="flex items-start gap-3 rounded-2xl border border-rose-200 bg-rose-50/70 p-4">
          <CircleAlert className="mt-0.5 h-5 w-5 flex-none text-rose-600" aria-hidden="true" />
          <div>
            <p className="text-xs font-extrabold text-rose-800">CNPJ histórico requer correção</p>
            <p className="mt-1 text-[11px] font-medium leading-relaxed text-rose-700">
              {empenhosWithInvalidCnpj} empenho(s) possuem CNPJ com formato ou dígitos verificadores inválidos.
              Os registros continuam visíveis para auditoria, mas novas operações dependentes da identidade do fornecedor serão bloqueadas até a correção.
            </p>
          </div>
        </div>
      )}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-1.5">
                      ...
                      {!report.cnpjValid && (
                        <span className="rounded-full border border-rose-200 bg-rose-50 px-1.5 py-0.5 text-[8px] font-extrabold uppercase tracking-wider text-rose-700">
                          DV inválido
                        </span>
                      )}
                    </div>
              ...
              <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-start">
                <div>
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    <p className="font-mono text-xs font-bold text-blue-100">
                      {formatSupplierCnpj(selectedSupplier.cnpj)}
                    </p>
                    {!selectedSupplier.cnpjValid && (
                      <span className="rounded-full border border-rose-200/60 bg-rose-500/20 px-2 py-0.5 text-[9px] font-extrabold uppercase tracking-wider text-rose-100">
