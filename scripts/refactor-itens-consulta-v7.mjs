...
const VIEW_PATH = 'features/itens/components/ConsultaItensView.tsx';

let source = fs.readFileSync(PAGE_PATH, 'utf8');
const originalLineCount = source.split('\n').length;

const startNeedle = "{activeTab === 'itens' && (() => {";
const nextMarker = '{/* TAB 7: GERENCIAR ITENS DO EMPENHO (Itens do Empenho - Screenshot 3) */}';
const start = source.indexOf(startNeedle);
if (start < 0) throw new Error('Bloco da Consulta Consolidada de Itens não encontrado.');
const markerIndex = source.indexOf(nextMarker, start);
if (markerIndex < 0) throw new Error('Marcador da tela Itens do Empenho não encontrado.');

const block = source.slice(start, markerIndex).trimEnd();
if (!block.endsWith('}')) throw new Error(`Final inesperado do bloco da Consulta de Itens: ${block.slice(-120)}`);

const andIndex = block.indexOf('&&');
let expression = block.slice(andIndex + 2).trim();
expression = expression.slice(0, -1).trim();

const parsedExpression = ts.createSourceFile('consulta-itens-expression.tsx', `const __view = ${expression};`, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
...
if (!initializer || parsedExpression.parseDiagnostics.length) {
  throw new Error(`Não foi possível analisar a expressão da Consulta de Itens: ${parsedExpression.parseDiagnostics.map(d => d.messageText).join('; ')}`);
}
...
const typeFor = (name) => {
  ...
  if (name === 'itensSearch') return 'string';
  if (name === 'itensSaldoFilter') return "'Todos' | 'Com saldo' | 'Sem saldo'";
  if (name === 'expandedConsultaItem') return 'string | null';
...
const viewFile = `${importLines.join('\n')}\n${contextInterface}/** Consulta consolidada de itens extraída sem alterar regras de negócio, dados ou persistência. */\nexport function ConsultaItensView({ context }: ConsultaItensViewProps) {\n  const { ${contextNames.join(', ')} } = context;\n  return ${expression};\n}\n`;
fs.mkdirSync(path.posix.dirname(VIEW_PATH), { recursive: true });
fs.writeFileSync(VIEW_PATH, viewFile);

const viewImport = "import { ConsultaItensView } from '../features/itens/components/ConsultaItensView';\n";
const importAnchor = "import { RelatoriosView } from '../features/relatorios/components/RelatoriosView';\n";
if (!source.includes(viewImport)) {
  if (!source.includes(importAnchor)) throw new Error('Âncora de importação RelatoriosView não encontrada.');
  source = source.replace(importAnchor, importAnchor + viewImport);
}

const replacementStart = source.indexOf(startNeedle);
if (replacementStart < 0) throw new Error('Bloco da Consulta de Itens desapareceu após inserir import.');
const replacementMarkerIndex = source.indexOf(nextMarker, replacementStart);
if (replacementMarkerIndex < 0) throw new Error('Marcador da tela Itens do Empenho desapareceu após inserir import.');
const replacement = `{activeTab === 'itens' && (\n            <ConsultaItensView context={{ ${contextNames.join(', ')} }} />\n          )}\n\n          `;
...
if (newLineCount < 3000 || newLineCount >= originalLineCount) throw new Error(`Guardrail: contagem de linhas inesperada (${originalLineCount} -> ${newLineCount}).`);
if (!source.includes(nextMarker)) throw new Error('Guardrail: marcador da tela Itens do Empenho desapareceu.');
...
console.log(`Consulta de Itens extraída com ${free.length} dependências livres, ${directImportNames.length} imports diretos e ${contextNames.length} props de contexto.`);
