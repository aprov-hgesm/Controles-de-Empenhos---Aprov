...
assert.match(expressUi, /warehouse-scanner-input/);
assert.match(expressUi, /SCAN → quantidade → ENTER/);
assert.match(expressUi, /warehouse-scanner-unknown/);
assert.match(expressUi, /warehouse-barcode-associate-save/);
assert.match(expressUi, /warehouse-outbound-fefo/);
assert.match(expressUi, /Usar lote recomendado/);
assert.match(expressUi, /warehouse-outbound-recent/);
assert.match(expressUi, /scannerRef\.current\?\.focus\(\)/);
...
assert.match(navigation, /href: '\/adm-deposito\/saida-expressa'/);
assert.match(navigation, /id: 'warehouseView'/);
assert.match(navigation, /warehouseView.*futurePhase: (?:'FASE 9'|null)/);
assert.match(sectionContent, /WarehouseExpressOutbound/);
assert.match(route, /WarehouseProtectedSurface section="outbound"/);
