...
const VIEW_PATH = 'features/relatorios/components/RelatoriosView.tsx';

let source = fs.readFileSync(PAGE_PATH, 'utf8');
const originalLineCount = source.split('\n').length;

const startNeedle = "{activeTab === 'relatorios' && (";
const nextMarker = '{/* CONSULTA CONSOLIDADA DE ITENS */}';
const start = source.indexOf(startNeedle);
if (start < 0) throw new Error('Bloco de Relatórios não encontrado.');
const markerIndex = source.indexOf(nextMarker, start);
if (markerIndex < 0) throw new Error('Marcador da Consulta Consolidada de Itens não encontrado.');

const block = source.slice(start, markerIndex).trimEnd();
if (!block.endsWith(')}')) throw new Error(`Final inesperado do bloco de Relatórios: ${block.slice(-120)}`);

const andIndex = block.indexOf('&&');
let expression = block.slice(andIndex + 2).trim();
expression = expression.slice(0, -1).trim();

const parsedExpression = ts.createSourceFile('relatorios-expression.tsx', `const __view = ${expression};`, ts.ScriptTarget.Latest, true, ts.ScriptKind.TSX);
...
if (!initializer || parsedExpression.parseDiagnostics.length) {
  throw new Error(`Não foi possível analisar a expressão de Relatórios: ${parsedExpression.parseDiagnostics.map(d => d.messageText).join('; ')}`);
}
...
importLines.push("import type { Empenho, Invoice } from '../../../lib/types';", '');

const typeFor = (name) => {
  if (name === 'empenhos') return 'Empenho[]';
  if (name === 'invoices') return 'Invoice[]';
  if (name === 'selectedReportInvoice') return 'Invoice | null';
  if (name === 'uniquePregaos') return 'string[]';
  if (name === 'editingNSId') return 'string | null';
  if (name === 'tempNSValue' || name === 'reportSearch' || name === 'reportStartDate' || name === 'reportEndDate' || name === 'relatoriosPregaoFilter') return 'string';
  if (name === 'showPdfModal') return 'boolean';
...
const viewFile = `${importLines.join('\n')}\n${contextInterface}/** Tela de Relatórios extraída sem alterar regras de negócio, persistência ou comportamento. */\nexport function RelatoriosView({ context }: RelatoriosViewProps) {\n  const { ${contextNames.join(', ')} } = context;\n  return ${expression};\n}\n`;
fs.mkdirSync(path.posix.dirname(VIEW_PATH), { recursive: true });
fs.writeFileSync(VIEW_PATH, viewFile);

const viewImport = "import { RelatoriosView } from '../features/relatorios/components/RelatoriosView';\n";
const importAnchor = "import { NotasFiscaisView } from '../features/notas-fiscais/components/NotasFiscaisView';\n";
if (!source.includes(viewImport)) {
  if (!source.includes(importAnchor)) throw new Error('Âncora de importação NotasFiscaisView não encontrada.');
  source = source.replace(importAnchor, importAnchor + viewImport);
}

const replacementStart = source.indexOf(startNeedle);
if (replacementStart < 0) throw new Error('Bloco de Relatórios desapareceu após inserir import.');
const replacementMarkerIndex = source.indexOf(nextMarker, replacementStart);
if (replacementMarkerIndex < 0) throw new Error('Marcador da Consulta de Itens desapareceu após inserir import.');
const replacement = `{activeTab === 'relatorios' && (\n            <RelatoriosView context={{ ${contextNames.join(', ')} }} />\n          )}\n\n          `;
...
if (newLineCount < 3500 || newLineCount >= originalLineCount) throw new Error(`Guardrail: contagem de linhas inesperada (${originalLineCount} -> ${newLineCount}).`);
if (!source.includes(nextMarker)) throw new Error('Guardrail: marcador da Consulta Consolidada desapareceu.');
...
console.log(`Relatórios extraído com ${free.length} dependências livres, ${directImportNames.length} imports diretos e ${contextNames.length} props de contexto.`);
