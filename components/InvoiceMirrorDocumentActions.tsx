...
interface InvoiceMirrorDocumentActionsProps {
  invoice: Invoice;
  user: User | null;
...
export function InvoiceMirrorDocumentActions({
  invoice,
  user,
  onDocumentUploaded,
  onNotify,
}: InvoiceMirrorDocumentActionsProps) {
  const inputRef = useRef<HTMLInputElement>(null);
  const [busyAction, setBusyAction] = useState<'upload' | 'view' | 'print' | 'download' | null>(null);
  const currentDocument = invoice.espelhoNotaFiscalPdf;

  const executeAction = async (action: 'view' | 'print' | 'download') => {
    if (!user || !currentDocument) return;
    setBusyAction(action);
    try {
      await runInvoicePdfAction(user, currentDocument, action, 'Espelho da Nota Fiscal');
    } catch (error) {
      onNotify(error instanceof Error ? error.message : 'Falha ao abrir o Espelho da Nota Fiscal.', 'error');
    } finally {
      setBusyAction(null);
    }
  };
  ...
  const handleFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      uploadedDocument = await uploadInvoiceMirrorPdf(user, invoice.empenhoId, invoice.id, file);
      await onDocumentUploaded(getInvoiceRecordKey(invoice), uploadedDocument);
      onNotify(
        currentDocument
          ? 'Nova versão do Espelho da Nota Fiscal anexada com segurança.'
          : 'Espelho da Nota Fiscal anexado com segurança.',
        'success'
      );
    } catch (error) {
      if (uploadedDocument) {
        await deleteInvoicePdfUpload(
          user,
          uploadedDocument.empenhoId,
          uploadedDocument.invoiceId,
          uploadedDocument.pathname
        ).catch(() => undefined);
      }
      onNotify(error instanceof Error ? error.message : 'Falha ao anexar o Espelho da Nota Fiscal.', 'error');
    } finally {
      setBusyAction(null);
    }
  };
  ...
  return (
    <section className="bg-white/80 backdrop-blur-md p-4 rounded-2xl border border-cyan-100 shadow-sm">
      <input
        ref={inputRef}
        type="file"
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h4 className="text-sm font-black text-[#0b1c30]">Documento — Espelho da Nota Fiscal</h4>
            {currentDocument ? (
              <>
                <p className="text-xs font-semibold text-gray-600 truncate mt-0.5">{currentDocument.originalName}</p>
                <p className="text-[10px] text-gray-400 font-semibold mt-1">
                  {formatBytes(currentDocument.size)} • {invoice.espelhoNotaFiscalPdfVersions?.length || 1} versão(ões) protegida(s)
                </p>
              </>
            ) : (
              <p className="text-xs text-gray-500 font-medium mt-0.5">
                Opcional. Quando anexado, entra automaticamente no consolidado de liquidação.
              </p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {currentDocument && (
            <>
              ...
              <button
                ...
                title="Imprimir Espelho da Nota Fiscal"
              >
              ...
              <button
                ...
                disabled={busyAction !== null}
                className="inline-flex items-center gap-1.5 px-2.5 py-2 rounded-lg border border-cyan-100 bg-cyan-50 text-cyan-700 hover:bg-cyan-100 disabled:opacity-50 text-xs font-bold transition-all"
                title="Baixar Espelho da Nota Fiscal"
              >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg bg-cyan-700 text-white hover:bg-cyan-800 disabled:opacity-50 text-xs font-bold transition-all"
          >
            {busyAction === 'upload' ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Upload className="w-3.5 h-3.5" />}
            {currentDocument ? 'Substituir PDF' : 'Anexar Espelho'}
          </button>
        </div>
      </div>
    </section>
