...
import type { EmpenhoPdfDocument } from '../../../lib/types';
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    ...
    const pathname = assertEmpenhoPath(url.searchParams.get('pathname'), empenhoId);
    ...
    console.error('Falha ao recuperar nota de empenho no Blob privado.');
    return Response.json(
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    const pathname = assertEmpenhoPath(body.pathname, empenhoId);
    const originalName = sanitizePdfFilename(body.originalName);
    const result = await get(pathname, { access: 'private', useCache: false });
    ...
    const isValidPdf =
      ...
      result.blob.size > 0 &&
      result.blob.size <= MAX_EMPENHO_PDF_BYTES &&
    ...
    const document: EmpenhoPdfDocument = {
      id: randomUUID(),
      pathname,
      originalName,
    ...
    console.error('Falha ao validar nota de empenho no Blob privado.');
    return Response.json(
...
export async function DELETE(request: Request): Promise<Response> {
  ...
  try {
    ...
    console.error('Falha ao remover upload incompleto do Blob privado.');
    return Response.json(
      { error: 'Não foi possível remover o upload incompleto.' },
      { status: 500, headers: { 'Cache-Control': 'no-store' } }
    );
  }
}
