...
## 13. Consolidação Módulo 5 / FASE 11.5 — contrato simplificado

O contrato externo oficial passou a ser `emprovex_siscofis_inventory_v1`, com somente `numeroItem`, `descricao`, `quantidade` e `valorUnitario`. UG e data-base são obtidas localmente; valor total é calculado pelo EMPROVEX. O Nº Ficha é auditável, mas nunca funciona como `materialId` ou chave única. Fichas repetidas não são consolidadas. O parser aceita apenas compatibilidades determinísticas de array puro na raiz e valor monetário brasileiro em string, sempre com aviso. O motor histórico de Marco Zero continua sendo a única autoridade para `INITIAL_BALANCE`, e snapshots posteriores continuam somente conciliatórios. Novos snapshots são `warehouse_siscofis_snapshot_v2`; documentos v1 continuam legíveis. O teste real de referência possuía 144 linhas e demonstrou a necessidade de revisão humana por haver uma divergência textual isolada na descrição. A prévia permite editar Nº Ficha, descrição, quantidade e valor unitário; qualquer edição invalida a confirmação até nova validação. Para material novo sem unidade disponível, o adaptador reutiliza o fallback canônico da integração de NF (`other` / `Apresentação não informada`) em vez de inventar uma unidade concreta. O ledger oficial já projeta `INITIAL_BALANCE` também em `UNASSIGNED`, portanto Marco Zero permanece coerente com o modelo atual sem segunda escrita de saldo.
